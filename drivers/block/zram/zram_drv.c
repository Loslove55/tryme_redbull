				...
				struct bio *bio, bool partial_io)
{
	...
	src = zpool_map_object(zram->mem_pool, handle, ZPOOL_MM_RO);
	if (size == PAGE_SIZE) {
	...
	zpool_unmap_object(zram->mem_pool, handle);
	zram_slot_unlock(zram, index);
...
compress_again:
	...
	alloced_pages = zpool_get_total_pages(zram->mem_pool) >> PAGE_SHIFT;
	...
	dst = zpool_map_object(zram->mem_pool, handle, ZPOOL_MM_WO);
